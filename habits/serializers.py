...
class HabitSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if data.get('reward') and data.get('related_habit'):
            raise serializers.ValidationError("Cannot set both 'reward' and 'related_habit'. Choose only one.")

        # Проверка: `time_to_complete` не должен превышать 120 секунд
        if data.get('time_to_complete', 0) > 120:
            raise serializers.ValidationError("The 'time_to_complete' cannot exceed 120 seconds.")
        ...
        if related_habit and not related_habit.is_pleasant:
            raise serializers.ValidationError("Only pleasant habits can be set as 'related_habit'.")
        ...
        if data.get('is_pleasant') and (data.get('reward') or data.get('related_habit')):
            raise serializers.ValidationError("Pleasant habits cannot have 'reward' or 'related_habit'.")
